...
function Post3({ id, username, userImg, img, caption }) {
    ...
    useEffect(
        () =>
            onSnapshot(
                query(
                    collection(db, 'posts3', id, 'comments'),
    ...
    useEffect(
        () =>
            onSnapshot(collection(db, 'posts3', id, 'likes'), (snapshot) =>
    ...
    const likePost = async () => {
        if (hasLiked) {
            await deleteDoc(doc(db, 'posts3', id, 'likes', session.user.uid))
        }
        else {
            await setDoc(doc(db, 'posts3', id, 'likes', session.user.uid), {
    ...
    useEffect(
        () =>
            onSnapshot(collection(db, 'posts3', id, 'ThumsUp'), (snapshot) =>
    ...
    const ThumsUpPost = async () => {
        if (hasThumsUp) {
            await deleteDoc(doc(db, 'posts3', id, 'ThumsUp', session.user.uid))
        }
        else {
            await setDoc(doc(db, 'posts3', id, 'ThumsUp', session.user.uid), {
                username: session.user.username
            });
        }
    };
    ...
    const sendComment = async (e) => {
        ...
        await addDoc(collection(db, 'posts3', id, 'comments'), {
